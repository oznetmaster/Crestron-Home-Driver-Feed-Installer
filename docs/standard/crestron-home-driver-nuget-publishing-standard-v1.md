...
This document defines the packaging standard used by this repository and by Crestron Home Driver Feed Installer.
...
A package is compliant with Crestron Home Driver NuGet Publishing Standard v1 only if all of the following are true:

- [ ] `PackageType` is `CrestronHomeDriver`
